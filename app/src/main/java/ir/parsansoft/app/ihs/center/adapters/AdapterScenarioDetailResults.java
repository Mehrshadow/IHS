...
public class AdapterScenarioDetailResults extends BaseAdapter {
    ...
    public AdapterScenarioDetailResults(Context context, Struct[] scenarioResults) {
        //super(context, R.layout.l_node_simple_key, nodes);
        this.context = context;
        this.scenarioItems = scenarioResults;
    }


    @Override
    public View getView(final int position, View convertView, ViewGroup parent) {
        G.log("AdapterScenarioDetailResults : Get View ... position :" + position);
        ...
        String description = G.T.getSentence(501) + " = " + scenarioItems[position].getValueName();
        if (scenarioItems[position].reverseTime > 0)
            description += "\n" + G.T.getSentence(511) + " " + Utility.readTime(scenarioItems[position].reverseTime);
        lo.txtValue.setText(description);

        lo.imgIcon.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View arg0) {
                Database.Results.delete(scenarioItems[position].iD);
                scenarioItems = Database.Results.select("ScenarioID=" + scenarioItems[position].scenarioID);
